...
void get_option()
{
	while (true)
	{
		...
		switch (c - 1)
		{
		case add_queue_element: add();
			break;
		case get_queue_element: get();
			break;
		case find_particular_element: find();
			break;
		case print_queue_elements: print();
			break;
		case check_sizeof_queue: check();
			break;
		case delete_queue: erase();
			break;
...
void get()
{
	...
	printf("Pobrano element : %s %s %d\n", students[0].name, students[0].surname, students[0].born);
...
void print()
{
	if (students != NULL)
	{
		printf("Aktualna zawartosc kolejki : \n");
		for (int i = 0; i<size; i++)
...
void erase()
{
	...
	size = 0;
	students = NULL;
	printf("Usunieto dane z kolejki.\n");
}
void my_exit()
{
...
void bin_out()
{
	FILE* file = fopen("kolejka.bin", "wb");
	if (file == NULL)
		error(5);
